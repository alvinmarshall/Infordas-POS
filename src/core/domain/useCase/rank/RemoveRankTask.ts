...
/**
 * RemoveRankTask class
 * class extends BaseUseCase {@Link ../base/BaseUseCase}
 */
@injectable()
export class RemoveRankTask extends BaseUseCase<any, string> {
  ...
  protected generateUseCase(input?: string | undefined): Promise<any> {
    if (input == null) throw new Error("id can't null");
    return this.rankRepository.removeRank(input);
  }
}
